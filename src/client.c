...
void handle_game_guess_response(Message *msg) {
  ...
  if (strncmp(msg->payload, "WIN", 3) == 0) {
    ...
    is_game_over = TRUE;
    is_win = (current_player == player_num);
  ...
  // Move to the next row for the next turn
  current_row++;

  // Clear the input field after the guess
  gtk_entry_set_text(word_entry, "");

  // Update the opponent's attempt count based on the current player
  opponent_attempts = (player_num == 1) ? p2_attempts : p1_attempts;

  // Update turn state
  gboolean is_our_turn = (current_player == player_num);
  gtk_widget_set_sensitive(GTK_WIDGET(word_entry), is_our_turn);
  gtk_widget_set_sensitive(GTK_WIDGET(submit_button), is_our_turn);

  // Display turn status
  g_print("Current Player: %d, Your Turn: %s\n", current_player, is_our_turn ? "Yes" : "No");

  // Show dialog only after UI updates
  if (is_game_over) {
    if (is_win) {
...
void on_LoginSubmit_clicked(GtkButton *button, gpointer user_data){
  ...
  int timeout = 0;
  Message response;
  while (timeout < 10) { // Try for 1 second
...
void init_game_board(GtkBuilder *builder){
  ...
  // Get the word entry widget reference
